...
impl StorageManager {
    ...
    fn format_qcow2_with_nbd(disk_image: &Path) -> Result<()> {
        ...
        let format_result = Command::new("mkfs.ext4")
            ...
            .output()
            .map_err(|e| {
                HypervisorError::InvalidConfig(format!("Failed to run mkfs.ext4: {}", e))
            });
    ...
    pub fn mount_disk_image(&self, instance_id: &str) -> Result<(PathBuf, String)> {
        ...
        let status = Command::new("qemu-nbd")
            .args(&[
                "--connect",
                &nbd_device,
                paths.disk_image.to_str().unwrap(),
            ])
            .status()
            .map_err(|e| {
                HypervisorError::InvalidConfig(format!("Failed to run qemu-nbd: {}", e))
    ...
    pub fn unmount_disk_image(&self, instance_id: &str, mount_point: &Path, nbd_device: &str) -> Result<()> {
        tracing::info!(
            instance_id = %instance_id,
            mount_point = %mount_point.display(),
            nbd_device = %nbd_device,
